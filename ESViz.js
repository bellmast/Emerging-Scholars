...
function drawNetwork(data) {
    ...
    for(i=0; i < yearAxis+1; i++) {
    	...
    	if(i==11){
    		...
    		pie = paper.piechart(yearXcoords[i], KJFFheight, KJFFscaleTracker[i]*1.5, [KJFFcarnegie1[i], KJFFcarnegie2[i]+KJFFcarnegiex[i]], {colors: cArray, sort:false})
    	}
		KJFFpieSet.push(pie)
    	...
    	if(i==10){
    		...
    		pie2 = paper.piechart(yearXcoords[i], KDFPheight, KDFPscaleTracker[i]*1.5, [KDFPcarnegie1[i], KDFPcarnegie2[i]+KDFPcarnegiex[i]], {colors: cArray, sort: false})
    	}
	...
	hideAllText = paper.text(50, 65, "hide all").hover(function() {
			linesSet.hide()
			allpieSet.hide()
			KDFPpieCheck = "no"
			KDFPpieCheck2 = "no"
